...
class IGTOnlyResearchAgent:
    ...
    def answer_query(self, query: str, max_iterations: int = 10,
        ...
        _print("    [concluding...]")
        return self._conclude_query(query, graph, trace, grambank_policy=grambank_policy)
    ...
    def _conclude_query(self, query: str, graph: EvidenceGraph, trace: list, grambank_policy: str = ""):
        from state import QueryResult
        ...
        grambank_reasoning = ""
        if grambank_policy:
            grambank_label, grambank_reasoning = self._generate_grambank_label(
                query=query,
        ...
        result = QueryResult(
            ...
            audit_objections=[],
            token_usage=self.llm.get_token_counts(),
            grambank_label=grambank_label,
            grambank_reasoning=grambank_reasoning,
        )
    ...
    def _generate_grambank_label(
        ...
        """
        Generate a Grambank label (single uppercase letter) in a dedicated call.
        Returns (grambank_label, grambank_reasoning).
        """
        evidence_str = "\n".join(f"- {ev}" for ev in (key_evidence or [])[:4]) \
        ...
        _print(f"    [generating Grambank label...]")
        raw    = self.llm.generate(prompt, max_new_tokens=256)
        result = self._parse_json(raw)
        label     = result.get("grambank_label",    "").strip()
        reasoning = result.get("grambank_reasoning", "")
        if not re.match(r'^[0-9?]$', label):
            for ch in label:
                ...
                label = '?'
        _print(f"    → label={label}  reasoning={reasoning[:80]}")
