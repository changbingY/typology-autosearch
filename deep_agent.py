...
{grambank_policy_block}{abbrev_legend}
...
CRITICAL rules for target_sections:
...
- If NO section summary explicitly mentions the queried phenomenon, set target_sections to []
  and rely on search_text queries in the ReAct loop to discover relevant sections instead.
  DO NOT fill target_sections with phonology, introduction, or other unrelated sections
  just to have something there — an empty list is correct when nothing is clearly relevant.

CRITICAL rules for igt_tags_to_check:
- Only use tags from STEP 1 above, or tags whose IGT statistics directly signal the phenomenon.
- If STEP 1 shows "LIKELY ABSENT", set igt_tags_to_check to [] and set
  category_absence_check to the relevant category instead.
- Do NOT copy generic high-frequency tags (LOC, IMPFV, DECL, etc.) that appear in IGT
  statistics but are unrelated to the queried phenomenon.
...
GRAMBANK CODING (only when a coding policy is provided below):
Read the policy carefully and assign the appropriate label based solely on
the evidence in your answer above. Do not infer from other features.
Output the label in "grambank_label" and a one-sentence justification in
"grambank_reasoning". If no policy is provided, omit these two fields.

{grambank_policy_block}
...
{{
  ...
  "grambank_label": "<digit or ? as defined in coding policy, e.g. 0, 1, 2, ? — ONLY if GRAMBANK CODING REQUIREMENT present in query>",
  "grambank_reasoning": "one sentence explaining the code — ONLY if GRAMBANK CODING REQUIREMENT present"
}}"""
...
GRAMBANK_LABEL_PROMPT = """You are applying a Grambank coding policy to a research answer.
...
Read the coding policy carefully and assign the correct label based ONLY on the
evidence in the answer above. Do not infer from other features or general knowledge.

Output ONLY valid JSON (no prose before or after):
{{
  "grambank_label": "<digit or ? exactly as defined in the coding policy, e.g. 0, 1, 2, ?>",
  "grambank_reasoning": "one sentence explaining why this label applies given the evidence"
}}"""


QUERY_RESCUE_PROMPT = """You are a linguistic typologist completing a partially-generated research report.

LANGUAGE: {language}
QUERY: {query}

RESEARCH ANSWER (already written):
{answer}

RAW EVIDENCE GATHERED (claims from the evidence graph):
{evidence_text}

The following three fields were not produced and must be generated now.

1. LINGUISTIC DEFINITION — a language-agnostic, typological definition of the phenomenon
   asked about in the query (1-2 sentences). Do NOT describe {language} specifically here.
   Example: "Definite articles are grammaticalized determiners that mark a nominal as
   identifiable or previously mentioned by the interlocutors."

2. STRUCTURAL DESCRIPTION — how this phenomenon is realised (or absent) specifically in
   {language}: mention concrete forms, markers, positions, or strategies used instead
   (2-3 sentences). Base this strictly on the research answer and evidence above.

3. KEY EVIDENCE — up to 4 items. Each item must be a single string in this format:
   "Finding: <what the evidence shows>. Source: §<Section name> [chunk_id] or IGT tool name. Justification: <why this supports or complicates the answer>."
   Rules:
   - Only cite sources that appear in the RAW EVIDENCE above.
   - Be specific: mention marker names, tag counts, or section names.
   - Do NOT invent chunk IDs or section names.

Output ONLY valid JSON:
{{
  "linguistic_definition": "language-agnostic typological definition (1-2 sentences)",
  "structural_description": "how this works specifically in {language} (2-3 sentences)",
  "key_evidence": [
    "Finding: ... Source: §Section name [chunk_id]. Justification: ...",
    "Finding: ... Source: IGT tag analysis: DEM. Justification: ..."
  ]
}}"""
...
class DeepLanguageResearchAgent:
    ...
    def _find_relevant_tags_via_llm(self, query_text: str) -> dict:
        """
        Ask the LLM to read this language's complete abbreviation table and
        infer which tags are functionally relevant to the queried phenomenon.

        This replaces the previous keyword-matching approach
        (find_tags_for_phenomenon) which could only find tags whose *name*
        contained the query words.  The LLM can reason across languages:
        e.g. recognising that DEM (demonstrative) may function as a definite
        marker even though the word "definite" does not appear in its meaning.

        Returns a dict with keys:
          relevant_tags  : list[str]  — tags worth investigating
          likely_absent  : bool       — True if the phenomenon is probably absent
          absence_reason : str        — explanation when likely_absent is True
          reasoning      : str        — brief justification for the tag choices
        """
        full_legend = self._abbrev.annotated_table()   # complete TAG → Meaning table

        stats = self.toolkit.igt_analyser.get_stats() if self._has_igt else None
        if stats:
            corpus_tags_str = ", ".join(sorted(stats.tag_profiles.keys()))
        else:
            corpus_tags_str = "(no IGT corpus loaded)"

        prompt = (
            f"You are a linguistic typologist.\n\n"
            f"PHENOMENON TO INVESTIGATE: {query_text}\n\n"
            f"This language's glossing abbreviations (TAG → Meaning):\n"
            f"{full_legend}\n\n"
            f"Tags confirmed present in this language's IGT corpus:\n"
            f"{corpus_tags_str}\n\n"
            f"Task:\n"
            f"1. Read every TAG and its Meaning carefully.\n"
            f"2. Using your typological knowledge, identify which tags in THIS "
            f"language's system could be functionally relevant to the phenomenon "
            f"\"{query_text}\". Consider indirect realisations — for example, "
            f"demonstratives that function as definite markers, specificity "
            f"clitics, applicatives that encode dative arguments, etc.\n"
            f"3. Only include tags that are in the 'Tags confirmed present' list "
            f"above, or tags worth checking for absence.\n"
            f"4. If NO tag in this system is plausibly related to the phenomenon, "
            f"set likely_absent=true and explain why.\n\n"
            f"Output ONLY valid JSON:\n"
            f"{{\n"
            f'  "relevant_tags": ["TAG1", "TAG2"],\n'
            f'  "reasoning": "one sentence explaining the connection",\n'
            f'  "likely_absent": true or false,\n'
            f'  "absence_reason": "explanation if likely_absent, else empty string"\n'
            f"}}"
        )
        ...
        parsed.setdefault("relevant_tags",  [])
        parsed.setdefault("reasoning",      "")
        parsed.setdefault("likely_absent",  False)
        parsed.setdefault("absence_reason", "")
        return parsed

    def _format_abbrev_candidates(self, query_text: str) -> str:
        """
        Use the LLM to read this language's abbreviation table and infer
        which tags are functionally relevant to query_text.

        The result is a formatted string injected into planning prompts so
        the planner starts from linguistically motivated tags rather than
        corpus-wide frequency rankings.
        """
        stats = self.toolkit.igt_analyser.get_stats() if self._has_igt else None

        # ── LLM-based tag discovery ───────────────────────────────
        llm_result = self._find_relevant_tags_via_llm(query_text)

        # ── Case 1: phenomenon likely absent in this language ─────
        if llm_result["likely_absent"]:
            absence_reason = llm_result["absence_reason"]
            lines = [
                "NOTE: LLM assessed this phenomenon as LIKELY ABSENT in this language.",
                f"Reason: {absence_reason}",
                "Recommended first tool: analyse_absence to confirm with corpus data.",
            ]
            return "\n".join(lines)

        # ── Case 2: relevant tags identified ─────────────────────
        relevant_tags = llm_result.get("relevant_tags", [])
        ...
        if relevant_tags:
            ...
            for tag in relevant_tags:
                ...
                if stats and tag in stats.tag_profiles:
                    ...
                    corpus_note = ""
                lines.append(f"  {label:<34}  {corpus_note}")
            return "\n".join(lines)

        # ── Case 3: LLM found nothing → fall back to translation search ──
        if not self._has_igt:
            return (
                "(LLM found no relevant tags in abbreviation table; "
        ...
        if not diag_tags:
            return (
                "(LLM found no relevant tags; translation search also found nothing — "
                "use search_translations(keyword) as first tool to explore the corpus)"
            )

        lines = [
            "NOTE: LLM found no direct abbreviation matches for this phenomenon.",
    ...
    def discover_domains(self) -> list:
        ...
        toc_with_summaries = self.toolkit.get_toc_with_summaries(
            skip_subsections=False, max_summary_chars=60,
        )
        ...
        prompt = DOMAIN_EXTRACTION_PROMPT.format(
            ...
            toc_with_summaries=toc_with_summaries,
    ...
    def _generate_grambank_label(
        ...
        """
        Dedicated single-purpose call that assigns a Grambank label (single uppercase letter).

        Separated from the main conclusion prompt so that:
          - The conclusion JSON is never truncated by having to also produce a label.
          - The label always has its own token budget (max_new_tokens=256).
          - The retry path (prose-only) also gets a label rather than leaving it empty.

        Called whenever grambank_policy is non-empty AND the main conclusion
        did not produce a valid grambank_label.
        """
        evidence_str = "\n".join(
            f"- {ev}" for ev in (key_evidence or [])[:4]
        ) or "(no key evidence recorded)"

        prompt = GRAMBANK_LABEL_PROMPT.format(
            language=self.language,
            query=query,
            answer=answer[:1500],
            key_evidence=evidence_str,
            grambank_policy=grambank_policy,
        )
        _print("       generating Grambank label (separate call)...", indent=2)
        raw    = self.llm.generate(prompt, max_new_tokens=256)
        result = self._parse_json(raw)
        result.setdefault("grambank_label",    "")
        result.setdefault("grambank_reasoning", "")
        label = result["grambank_label"].strip()
        if not re.match(r'^[0-9?]$', label):
            for ch in label:
                ...
                label = '?'
        result["grambank_label"] = label
        _print(
            f"       → label={result['grambank_label']}  "
            f"reasoning={result['grambank_reasoning'][:80]}",
            indent=2,
        )
        return result
    ...
    def build_query_plan(self, query: str, grambank_policy: str = "") -> dict:
        ...
        # Inject the Grambank coding policy at the top of the planning prompt so
        # the model knows exactly what evidence it needs to find.  The full policy
        # (including scoring rules) is shown here deliberately: knowing the
        # decision criteria helps the planner target the right sections and tags
        # rather than doing generic search.
        if grambank_policy:
            grambank_policy_block = (
                "GRAMBANK CODING POLICY (read before planning — this defines what evidence you need to find):\n"
                f"{grambank_policy.strip()}\n\n"
            )
        else:
            grambank_policy_block = ""
        ...
        prompt = QUERY_PLAN_PROMPT.format(
            language=self.language,
            query=query,
            grambank_policy_block=grambank_policy_block,
            abbrev_legend=abbrev_legend,
            abbrev_candidates=abbrev_candidates,
            toc_with_summaries=toc_with_summaries,
            igt_summary=igt_summary[:2000],
    ...
    def answer_query(self, query: str, max_iterations: int = 8,
                     grambank_policy: str = "") -> dict:
        W = 60
        _print(f"\n{'='*W}")
        ...
        plan = self.build_query_plan(query, grambank_policy=grambank_policy)
        ...
        if grambank_policy:
            grambank_policy_block = (
                f"CODING POLICY (apply after writing the answer):\n{grambank_policy}\n"
            )
        else:
            grambank_policy_block = ""
        ...
        # max_new_tokens raised to 4096 so the full JSON (answer + key_evidence +
        # grambank fields) is not truncated by the token budget.
        conclusion_raw = self.llm.generate(conclusion_prompt, max_new_tokens=4096)
        conclusion     = self._parse_json(conclusion_raw)
        ...
        # ── Rescue missing structured fields (retry path or truncated JSON) ──
        # When the main JSON is truncated or the retry path is used, key_evidence,
        # linguistic_definition, and structural_description are all lost.
        # Generate them in a single dedicated call so the output always has
        # traceable citations and the definition/realisation sections.
        needs_rescue = (
            not conclusion.get("key_evidence")
            or not conclusion.get("linguistic_definition", "").strip()
            or not conclusion.get("structural_description", "").strip()
        )
        if needs_rescue:
            missing = [
                f for f, v in [
                    ("key_evidence",         conclusion.get("key_evidence")),
                    ("linguistic_definition", conclusion.get("linguistic_definition", "").strip()),
                    ("structural_description", conclusion.get("structural_description", "").strip()),
                ]
                if not v
            ]
            _print(
                f"       rescuing missing fields: {missing}...",
                indent=2,
            )
            ev_text = "\n".join(
                f"- [{c.claim_type.value}] conf={c.confidence:.2f} | {c.text} "
                f"(source: {c.source})"
                f"{' | text: ' + c.raw_evidence[:150].replace(chr(10),' ') if c.raw_evidence else ''}"
                for c in graph.claims.values()
            )[:3000]
            rescue_prompt = QUERY_RESCUE_PROMPT.format(
                language=self.language,
                query=query,
                answer=conclusion.get("answer", "")[:800],
                evidence_text=ev_text,
            )
            rescue_raw    = self.llm.generate(rescue_prompt, max_new_tokens=1024)
            rescue_parsed = self._parse_json(rescue_raw)
            if not conclusion.get("key_evidence"):
                conclusion["key_evidence"] = rescue_parsed.get("key_evidence", [])
            if not conclusion.get("linguistic_definition", "").strip():
                conclusion["linguistic_definition"] = rescue_parsed.get("linguistic_definition", "")
            if not conclusion.get("structural_description", "").strip():
                conclusion["structural_description"] = rescue_parsed.get("structural_description", "")
            _print(
                f"       → rescued: "
                f"key_evidence={len(conclusion['key_evidence'])}  "
                f"definition={'✓' if conclusion.get('linguistic_definition') else '✗'}  "
                f"description={'✓' if conclusion.get('structural_description') else '✗'}",
                indent=2,
            )

        # ── Grambank label (separate dedicated call) ──────────────────────────
        # The main conclusion prompt may have been truncated (JSON cut off before
        # grambank_label was written), or the prose retry path never produces a
        # label at all.  Either way, if grambank_policy was supplied and the label
        # is still missing, generate it now in its own short call with its own
        # token budget — completely independent of the answer generation above.
        if grambank_policy and not conclusion.get("grambank_label", "").strip():
            label_result = self._generate_grambank_label(
                query=query,
                answer=conclusion.get("answer", ""),
                key_evidence=conclusion.get("key_evidence", []),
                grambank_policy=grambank_policy,
            )
            conclusion["grambank_label"]    = label_result["grambank_label"]
            conclusion["grambank_reasoning"] = label_result["grambank_reasoning"]
        ...
        result = QueryResult(
            ...
            audit_objections=audit.get("objections", []),
            token_usage=self.llm.get_token_counts(),
            grambank_label=conclusion.get("grambank_label", ""),
            grambank_reasoning=conclusion.get("grambank_reasoning", ""),
        )
        ...
        if result.grambank_label:
            _print("  │", indent=2)
            _print(f"  │  GRAMBANK LABEL : {result.grambank_label}", indent=2)
            _print(f"  │  GRAMBANK REASON: {result.grambank_reasoning[:100]}", indent=2)
            if grambank_policy:
                _print("  │", indent=2)
                _print("  │  CODING POLICY:", indent=2)
                for line in grambank_policy.strip().splitlines():
                    _print(f"  │    {line[:110]}", indent=2)
        _print("  └─" + "─" * W, indent=2)
        _print("", indent=0)
