...
class IGTAnalyser:
    ...
    def query_tag(self, tag: str) -> str:
        ...
        # Try exact match first (handles lowercase lexical stems)
        if tag in stats.tag_profiles:
            return self._format_profile(stats.tag_profiles[tag])
        tag_u = tag.upper()
        if tag_u not in stats.tag_profiles:
            # Try case-insensitive prefix match
            matches = [t for t in stats.tag_profiles if t.upper().startswith(tag_u)]
            if not matches:
    ...
    def analyse_semantic_context(self, tag: str, max_examples: int = 8) -> str:
        ...
        if tag in stats.tag_profiles:
            tag_u = tag
        elif tag_u not in stats.tag_profiles:
            matches = [t for t in stats.tag_profiles if t.upper().startswith(tag_u)]
            if not matches:
    ...
    def get_triline_examples(self, query: str, max_examples: int = 8) -> str:
        ...
        # 1. Tag match (exact first, then uppercase for grammatical tags)
        matches = [ex for ex in self.examples
                   if query in ex.gloss_tags or query.upper() in ex.gloss_tags]
        source  = f"tag={query}"
